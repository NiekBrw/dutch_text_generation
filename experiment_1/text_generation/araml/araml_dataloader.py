    ...
    def create_batches_augment(self, raml_file, LM_model_fw, LM_model_bw, sess, PAD_ID):
        if os.path.exists(raml_file):
            ...
            for sent in self.token_stream:
                ...
                sampling_len = np.random.choice(list(range(tgt_sent_len + 1)), p=payoff_prob[tgt_sent_len],
                ...
                for i, e in enumerate(sampling_len):
                    if e > 0:
                        ...
                        if e > tgt_sent_len:
                            old_word_pos = np.random.choice(list(range(tgt_sent_len)), size=e)
                        else:
                            old_word_pos = np.random.choice(list(range(tgt_sent_len)), size=e, replace=False)
                        ...
                        for j in old_word_pos:
                            ...
                            word_pos = np.random.choice(list(range(self.vocab_size)), p=value_topk)
                            num_sampling = 0
